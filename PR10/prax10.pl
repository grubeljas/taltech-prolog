...
vota(X,Y,Suund,X1,Y1,X2,Y2) :-
    leia_suund(MyColour, Suund),
    enemy(MyColour, EnemyColour),
...
tee_kaik(X, Y, X1, Y1) :-
    ruut(X, Y, A), ruut(X1, Y1, 0),
    retractall(ruut(X, Y, A)), retractall(ruut(X1, Y1, 0)),
    assert(ruut(X1, Y1, A)), assert(ruut(X, Y, 0)).
